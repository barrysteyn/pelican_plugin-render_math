...
# Global variables
color = "black" # Note: not set as #000 so as to aide in readbility
...
latexScript = lambda: """
    <script type= "text/javascript">
        ...
        s[(window.opera ? "innerHTML" : "text")] =
            ...
            "    }, " +
            "    'HTML-CSS': { " +
            "        styles: { '.MathJax .mo, .MathJax .mi': {color: '""" + color + """ ! important'}} " +
            "    } " +
            "}); ";
...
def wrap(content):
    ...
    try:
        ...
        wrapInTags.latexRe = re.compile(r'(\$\$|\$).*?\1|\\begin\{(.+?)\}.*?\\end\{\2\}', re.DOTALL | re.IGNORECASE)

    def mathTagWrap(match):
        return '<mathjax>'+match.group(0)+'</mathjax>'

    return applyTypogrify.latexRe.subn(mathTagWrap, content) 
...
def wrapMathInTags(instance):
    ...
    instance._content = wrap(instance._content)[1]
...
def applyTypogrify(instance):
    ...
    numSubs, instance._content = wrap(instance._content)
